...
#define     SQL_UPDATE_GROUP                @"REPLACE INTO %@ ( uid, gid, name, avatar, ext1, ext2, ext3, ext4, ext5) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ? )"

#define     SQL_SELECT_GROUPS               @"SELECT * FROM %@ WHERE uid = %@"

#define     SQL_DELETE_GROUP                @"DELETE FROM %@ WHERE uid = '%@'"
...
#define     SQL_DELETE_GROUP_MEMBER             @"DELETE FROM %@ WHERE uid = '%@'"
